...
import { assetRoutes, startDocumentDeletion } from "./tldraw-assets";
import {
  authMeAndLogoutRoutes,
  registerApiAuth,
...
// OAuth provider routes (browser redirects, no app-side fetch — kept
// off the typed chain).
registerOAuthProviderRoutes(app);
// Auth middleware mutates `app` to attach a `/api/*` gate. Done as a
// side effect so the chain below stays a clean route definition.
registerApiAuth(app);

// Workspace + per-document routes as a chained sub-router. Defining
// them as one Hono chain (not a sequence of `app.<method>(...)`
// statements) is what lets `typeof routes` carry the full path/param/
// per-status-response signature for the app's typed RPC client.
//
// Status literals (the second arg to `c.json(...)`) are critical:
// the client narrows `res.json()` per status, but only when the
// server passes the literal. `c.json(payload)` widens to
// `ContentfulStatusCode` and the narrowing degrades to a flat union
// over all 2xx bodies.
const documentRoutes = new Hono<AppBindings>()
  ...
  .put(
    ...
    // is fully buffered before this point — gating on
    // content-length here keeps a runaway client from streaming
    // multi-MB blobs into the parser. Defense in depth: content-length
    // can be omitted, in which case the schema validator's own size
    // limits and the platform-level body limit are the next gates.
    async (c, next) => {
...
// One big chain that captures every typed-RPC route the app cares
// about. `typeof routes` is what the app's `hc<AppType>()` client
// consumes — adding a route means adding it to one of the chained
// sub-routers above (then the `.route()` mount picks up its types
// automatically).
//
// The websocket upgrade (`/api/connect/:documentId`) is registered
// outside the chain via `app.get(...)` because it returns a raw
// `Response` for the WS handshake; the typed client doesn't reach
// websockets anyway (clients open them via the `WebSocket`
// constructor with the URL).
const routes = app
  .route("/", authMeAndLogoutRoutes)
  .route("/", identitiesRoutes)
  .route("/", assetRoutes)
  .route("/", documentRoutes);

export type AppType = typeof routes;

// WebSocket upgrade for sync — kept off the typed chain (returns a
// raw `Response` for the handshake; the app opens this via the
// browser `WebSocket` constructor, not the RPC client).
