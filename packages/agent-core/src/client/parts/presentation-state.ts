...
import { deriveTimelineFromShapes, type FrameAction } from "anipres/models";
import {
  FocusedEasingSchema,
  type FocusedEasing,
...
function summarise(editor: Editor): {
  ...
  const steps = timeline.steps.map((step, zeroBased) => ({
    index: zeroBased + 1,
  ...
  return { totalSteps: timeline.steps.length, steps };
}
