...
packages/worker/
  src/
    ...
    WorkspaceFeedRoom.ts     # Per-workspace DurableObject backing the SSE feed (subscribers list + fanOut, no persisted state); bumpWorkspaceFeed helper for mutation routes
...
Registering a record type is therefore a one-way step for the deployment: the moment a document holds one, a worker built without that registration can no longer load the room. Releases that add record types roll forward only, and every later release must keep the registration. Ordering is not a concern here — the worker serves the app bundle (`[assets]` in `wrangler.toml`, with `run_worker_first` carving out `/api/*` and `/auth/*`), and `deploy-worker` ships the worker and `app-dist` from one build, so the two can never be out of step with each other. The client-side counterpart is `SYNC_CLIENT_VERSION` (see [`design-animation-data-model.md`](./design-animation-data-model.md) § Risk 6), which keeps a client whose vocabulary doesn't match this build out of the document; in practice that means a tab still running the previous bundle, which recovers by reloading.
