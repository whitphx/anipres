...
**Obsolete since Animation Data Model v2** (`globalIndex` and `getNextGlobalIndexFromCueFrames` no longer exist; both sides now append a fresh step via `orderKeyBetween`): there was a parallel, related divergence on the _creation_ side worth bundling into the same fix: `Anipres.tsx`'s slide auto-attach handler picks `globalIndex: orderedSteps.length` (count of currently-rendered steps), while the agent's `buildAutoCameraCueFrame` and `attachCueFrame` apply paths use `getNextGlobalIndexFromCueFrames`, which is `Math.max(...existingIndexes) + 1`. Both are correct on a gap-free timeline; they disagree when a deletion left a hole (the React side picks the count, skipping the gap; the agent side picks max+1, preserving the gap). Extract a single canonical `nextGlobalIndex` helper alongside the deletion-reconciliation extraction so both sides agree.
...
- **Wire `onError` on the worker path.** The MCP edit tool passes an `onError` callback into `streamActions` so the AI SDK's swallowed-error events surface (this is what unblocked diagnosing the `invalid x-api-key` no-op during PR #477). The worker route never wires `onError`, so any provider error the SDK reports via `onError` instead of throwing out of the iterator (rate limits, transient 5xx, billing rejections) lands as an empty stream that looks identical to a clean model refusal — the chat panel renders nothing useful and the user can't tell what went wrong. Required for good UX before this is exposed broadly: pass an `onError` that the route surfaces in an SSE `error` frame (subject to the sanitization above).
