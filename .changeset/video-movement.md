...
**Deploy note.** The new vocabulary lives entirely in `meta`, which older builds ignore, so no document becomes unloadable, and media events are still written with the legacy `media-control` binding alongside their own target key so an older build can resolve them rather than deleting them as orphans. `SYNC_CLIENT_VERSION` moves to 4 regardless, since an older client has no notion of a video that moves: it mounts a player per carrier and keeps them all visible, so a moving video comes up as several independent players. The gate keeps such a client away only while it disagrees with the worker, so rolling the deployment back to a release that predates this one will misrender presentations authored since, though nothing is lost and they render correctly again on roll-forward.
