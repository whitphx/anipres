...
The shared-room arbitration for deleting a video's last carrier is designed but not built, so removing the event markers of a deleted video runs only where the client is the document's sole writer; in a synced document those markers are left in place rather than risking their loss to a concurrent edit. Carrying a video's settings onto the surviving carriers is not gated that way and runs wherever a carrier is deleted, since deleting the carrier whose revisions won a setting would otherwise drop the video back to a stale carrier's older value.
