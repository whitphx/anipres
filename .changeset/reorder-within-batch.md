...
Unlike a drag across steps, which pushes the frames behind it along and sweeps up the same-track batches it passes, this moves exactly the frame being dragged. The timeline shows the difference while the drag is in progress: the neighbouring frames shift aside for a drag that would carry them along, and stand still once the drop under the pointer would reorder instead, leaving the outlined frame as the one whose place is being taken.
